/**
 * SearXNG for dsh: two capabilities over one self-hosted instance.
 *
 * 1. Search provider on `ctx.web` 鈥?routes the model-facing `web_search` tool
 *    to the instance. The instance must enable JSON output (its `search.formats`
 *    must contain `json`).
 * 2. `searxng_scholar` tool on `ctx.tools` 鈥?Google Scholar aggregated by the
...
/** Output schema for the searxng_scholar tool (dsh-tools schema DSL). */
const SCHOLAR_OUTPUT_SCHEMA = {
  type: 'object',
  additionalProperties: false,
  properties: {
    query: { type: 'string', required: true, description: 'The executed query.' },
    engines: { type: 'string', required: true, description: 'SearXNG engines the query was pinned to.' },
...
/** Render scholar results as compact model-facing text. */
function renderScholarResults(value) {
  const lines = [`SearXNG (${value.engines}) 鈥?${value.total} result(s) for "${value.query}"${value.page !== undefined ? `, page ${value.page}` : ''}${value.truncated ? ' [truncated]' : ''}`];
...
/**
 * Register the provider on `ctx.web` and the scholar tool on `ctx.tools`.
 * Reads only its row config. Misconfiguration fails at load.
 *
 * Row config keys: `baseURL` (required); `engines` and `language` (optional
 * SearXNG filters for the web provider, comma-list and BCP-47);
 * `scholarEngines` (optional, default `"google scholar"`), and
 * `timeoutMs` (optional, default 30000) for the scholar tool.
 *
 * @param ctx - plugin context providing `ctx.web` and `ctx.tools`.
 * @param config - the row's config described above.
 * @throws when `baseURL` is missing or blank, or `timeoutMs` is not a positive number.
 */
export function apply(ctx, config) {
  ...
  ctx.tools.register(defineTool({
    ...
    output: {
      schema: SCHOLAR_OUTPUT_SCHEMA,
      render: (_args, value) => renderScholarResults(value),
    },
    timeoutMs,
    isConcurrencySafe: () => true,
    presentCall: (args) => `searxng_scholar: ${args.query}`,
