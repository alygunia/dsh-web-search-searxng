 ...
 * 2. `searxng_scholar` tool on `ctx.tools` — scholarly search pinned to the
 *    instance engines named by `scholarEngines` (default `google scholar`;
 *    this deployment pins `pubmed,arxiv,google scholar`), a credit-free
 *    complement to the dsh-ai4scholar tools (which stay authoritative for
 *    citation counts, abstracts, and citation-graph traversal).
...
export function apply(ctx, config) {
  ...
  const makeSearxngTool = ({ name, description, engines }) => defineTool({
    ...
    output: {
      ...
      // defineTool's contract is render(): ContentBlock[] (dsh-tools schema.d.ts).
      // A bare string here passes the runtime's lossless-JSON snapshot unchallenged,
      // lands in the durable tool/result as `content: "<text>"`, then crashes the
      // NEXT request build ("content.some is not a function") and makes the whole
      // session log fail restore validation — every turn after the first call dies
      // and the session can no longer be restored.
      render: (_args, value) => [{ type: 'text', text: renderSearxngResults(value) }],
      // presentResult receives a ToolResult ({ content, isError, meta }) — NOT the
      // canonical value — so every field a completed card needs must cross this
      // projector first. Reading the value's fields off `result` silently renders
      // "undefined result(s)" (and shows a card for failures instead of falling
      // back to the generic error card).
      presentationMeta: (_args, value) => ({ total: value.total, engines: value.engines }),
    },
    timeoutMs,
    isConcurrencySafe: () => true,
    presentCall: (args) => ({ card: 'generic', title: `${name}: ${args.query}`, kind: 'search', rawInput: args.query }),
    presentResult: (args, result) => {
      if (result.isError) return undefined;
      const meta = result.meta;
      if (meta === null || typeof meta !== 'object' || Array.isArray(meta)) return undefined;
      const { total, engines } = meta;
      if (typeof total !== 'number' || typeof engines !== 'string') return undefined;
      return { card: 'generic', title: `${total} result(s) for "${args.query}" via SearXNG (${engines})` };
    },
